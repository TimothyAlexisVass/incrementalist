...
## Objective
Port the Chest Draw mini-game and establish the core `daily_bonus.play` command flow, ensuring server-authoritative rules and WebGL rendering.

## Implementation

### 1. Command & State Orchestration
- **Command**: Implement `daily_bonus.play { game: "chest_draw" }`.
...
- **State Merging**: 
  - Update `applyAuthoritativeData` in `assets/src/net/snapshots.ts` to merge the `daily_bonus` result slice.
  - Ensure `coins`, `exp`, `shards`, and `cores` are updated from the `reward` field in the result.

### 2. Server Rules (`chest_draw.ex`)
- **Location**: `lib/incrementalist/game/features/daily_bonus/games/chest_draw.ex`.
...
- **Interaction**:
  - Clicking the chest box triggers the `daily_bonus.play` command.
- **Reveal sequence**:
