...
| --- | --- | --- | --- | --- |
| `global push` | Server -> all clients | Shared global ticks (season/time/rain/temp) | No | No |
| `player push` | Server -> one client | Recurring per-player pushes (soil/projection params/etc.) | No | No |
...
- Payload:
  ...
  - `server_time` (ISO8601 UTC)
  - `soil` (visible soil state; full payload every minute)
  - optional recurring player fields (if needed): future non-command projection fields only.
  - `has_bonustime_token` is change-only: include only when server value changed.
- Cadence: every UTC minute boundary (`...:00.000Z`) aligned to absolute time, not process-relative intervals.
...
### Backend Plan
1. Add push-event allowance to protocol docs/AGENTS (explicit exception to command/result-only for non-durable recurring/global ticks).
2. Add global scheduler process for `global.tick` at UTC minute boundaries.
3. Add per-player recurring projection tick scheduling in `PlayerServer` at UTC minute boundaries.
4. Route push events through channel process using PubSub fanout + Phoenix push (not `phx_reply`).
...
### Frontend Plan
1. Extend socket message handling to process server-pushed events (currently only `phx_reply` is handled).
2. Add typed handling for `global.tick` and `player.projection.tick`.
...
3. **Global Tick Live**
   - Ship `global.tick`.
4. **player Tick Live**
   - Ship `player.projection.tick` for soil/projection recurrence.
5. **Immediate Cutover**
...
### Testing Plan
- Backend:
  - Unit tests for UTC boundary scheduling for both global and player ticks.
...
1. Should `global.tick` and `player.projection.tick` share one event (`projection.tick`) with scoped payload, or stay separate for clarity?
   - Decision: Keep two separate events for clear authority boundaries and simpler payload contracts.
2. Should PlayerServer push strictly hourly only, or also at non-hour recurring boundaries (e.g. BonusTime slot boundaries) in same lane?
   - Decision: Push `player.projection.tick` every UTC minute boundary.
...
6. What transport pattern should carry push events to sockets?
   - Decision: PubSub fanout. `GameChannel` subscribes on join and relays via `push/3`; schedulers/PlayerServer broadcast to global/player topics.
...
11. Should `has_bonustime_token` be included every minute in `player.projection.tick`?
...
22. Should `soil` be change-only or always included on player minute ticks?
   - Decision: Always include full `soil` on every minute `player.projection.tick`.
