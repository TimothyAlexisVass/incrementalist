...
import { handleItemChecklistInteractions, getItemChecklistState, ItemChecklistState } from "./11-item-checklist/interactions";
import { getItemChecklistData } from "./11-item-checklist/view-model";
import { getPlinkoDropData } from "./05-plinko-drop/view-model";
import { handlePlinkoDropInteractions, getPlinkoState, PlinkoState } from "./05-plinko-drop/interactions";
import { getJackpotMeterData } from "./14-jackpot-meter/view-model";
import { handleJackpotMeterInteractions, getJackpotState, JackpotState } from "./14-jackpot-meter/interactions";
...
import { getItsBonusTimeData } from "./15-its-bonus-time/view-model";
import { handleItsBonusTimeInteractions, getItsBonusTimeState, ItsBonusTimeState } from "./15-its-bonus-time/interactions";
...
import { handleScratchCardInteractions, getScratchCardState, ScratchCardState } from "./12-scratch-card/interactions";
import { getLuckyDiceData } from "./10-lucky-dice/view-model";
import { getLuckyDiceState, handleLuckyDiceInteractions, LuckyDiceState } from "./10-lucky-dice/interactions";
import { getHammerSmashData } from "./06-hammer-smash/view-model";
import { getHammerSmashState, handleHammerSmashInteractions, HammerSmashState } from "./06-hammer-smash/interactions";
...
export function handleBonusTimeInteractions(
  ...
  if (activeGameId === 'chest_draw') {
    ...
    if (data) {
      const intent = handleScratchCardInteractions(input, data, gameRect, channel, runCommand);
    ...
    if (data) {
      const intent = handleLuckyDiceInteractions(input, data, gameRect, channel, runCommand);
    ...
    if (data) {
      const intent = handleHammerSmashInteractions(input, data, gameRect, channel, runCommand);
