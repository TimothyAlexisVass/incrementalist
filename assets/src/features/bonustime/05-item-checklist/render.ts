...
import { ItemChecklistState, getItemChecklistState } from "./interactions";
...
export function renderItemChecklist(
  ...
  const state = getItemChecklistState();
  ...
  let instruction = "";
  if (state === ItemChecklistState.IDLE) {
    instruction = data.hasToken ? "CLICK ANYWHERE TO CHECK OFF" : "NO TOKENS AVAILABLE";
  } else if (state === ItemChecklistState.REVEALING) {
    instruction = "CHECKING OFF...";
  } else if (state === ItemChecklistState.REVEALED) {
