defmodule Incrementalist.Game.Features.Achievements.Rules do
  ...
  def evaluate(%State{} = state) do
    ...
    {updated_achievements, unlocked_count, unlocked_favor} =
      Enum.reduce(defs, {state.achievements || %{}, 0, 0}, fn achievement_def,
  ...
  defp condition_met?(condition, state) do
    case condition do
      "tutorial_graduated" ->
        state.stats.tutorial_graduated
      ...
      "rewards_50" ->
        state.progress_bar.rewards_claimed >= 50

      "rewards_250" ->
        state.progress_bar.rewards_claimed >= 250

      "rewards_500" ->
        state.progress_bar.rewards_claimed >= 500

      "rewards_1000" ->
        state.progress_bar.rewards_claimed >= 1000

      "coins_50000" ->
        BigNum.compare(state.stats.total_coins_earned, BigNum.from_number(50_000)) >= 0

      "coins_100000" ->
        BigNum.compare(state.stats.total_coins_earned, BigNum.from_number(100_000)) >= 0

      "shards_2500" ->
        BigNum.compare(state.stats.total_shards_earned, BigNum.from_number(2500)) >= 0

      "cores_100" ->
        BigNum.compare(state.stats.total_cores_earned, BigNum.from_number(100)) >= 0

      "screens_viewed_stats" ->
        state.stats.screens_viewed_stats

      "screens_viewed_quests" ->
        state.stats.screens_viewed_quests

      "screens_viewed_achievements" ->
        state.stats.screens_viewed_achievements
